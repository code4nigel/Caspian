...
<body>
  ...
  <div class="hub-container">
    ...
    <div class="brand-sub">SELECT AI PLATFORM &bull; V1.0.26</div>
  ...
  <script>
    try {{
      if (window.CaspianBridge && typeof window.CaspianBridge.getAppVersion === 'function') {{
        var v = window.CaspianBridge.getAppVersion();
        var subEl = document.querySelector('.brand-sub');
        if (subEl) subEl.textContent = 'SELECT AI PLATFORM • V' + v;
      }}
    }} catch(e) {{}}
...
with open('d:/Projects/Chatgpt Pruner/Caspian-Android/assets/launch_hub.html', 'w', encoding='utf-8') as out:
    out.write(html_content)

print("Successfully updated launch_hub.html with dynamic getAppVersion sync!")
