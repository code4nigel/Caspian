...
// Robust Tab Capture Helper with Quota Rate Limit Backoff (Firefox & Chromium Compatible)
async function captureTabWithQuotaRetry(windowId, maxRetries = 5) {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      const dataUrl = await new Promise((resolve, reject) => {
        const callback = (resUrl) => {
      ...
      // Wait for quota window to clear
...
async function performRippleFrameCapture(tabId, options = {}) {
  ...
  try {
    ...
    if (!isFullPage) {
      ...
      // Multi-step Progressive Scrolling
      let y = 0;
      let sliceIndex = 0;
      const maxSlices = 100;
