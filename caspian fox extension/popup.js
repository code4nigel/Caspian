...
// Settings Backup & Sync (Export / Import JSON) - 100% Cross-Compatible (Chromium & Firefox)
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', () => {
      ...
      chrome.storage.local.get(null, (storedData) => {
        const raw = storedData || {};

        // 1. Build a clean, complete settings dictionary by merging DEFAULTS and active storage
        const cleanSettings = { ...DEFAULTS };

        for (const [key, val] of Object.entries(raw)) {
          // Strictly exclude temporary session caches and binary blobs
          if (
            key === 'latest_rippleframe_capture' ||
            key === 'pendingTransferContext' ||
        ...
        // Guarantee synchronization of legacy and modern pruner keys
        if (cleanSettings.pruningEnabled !== undefined) {
          cleanSettings.enabled = Boolean(cleanSettings.pruningEnabled);
        } else if (cleanSettings.enabled !== undefined) {
          cleanSettings.pruningEnabled = Boolean(cleanSettings.enabled);
        }

        const manifestVer = chrome.runtime?.getManifest?.()?.version || '6.3.0';
        const exportObj = {
          app: 'Caspian',
          format: 'caspian_settings_v1',
          version: manifestVer,
        ...
        const filename = `caspian_settings_backup_${dateStr}.json`;
        ...
        if (chrome.downloads && typeof chrome.downloads.download === 'function') {
          chrome.downloads.download({
            ...
            if (chrome.runtime.lastError) {
              // Fallback to synthetic anchor download if download API failed or was cancelled
              const a = document.createElement('a');
              a.href = blobUrl;
              a.download = filename;
              document.body.appendChild(a);
        ...
        setTimeout(() => { exportBtn.innerHTML = origHtml; }, 1600);
      });
    });
  }
    ...
    fileInput.addEventListener('change', (e) => {
      ...
      reader.onload = (event) => {
        try {
          ...
          // Support all standard, mobile, and legacy backup shapes
          let incoming = null;
          if (parsed && typeof parsed === 'object') {
            if (parsed.settings && typeof parsed.settings === 'object') {
              incoming = parsed.settings;
            } else if (parsed.caspianSettings && typeof parsed.caspianSettings === 'object') {
              incoming = parsed.caspianSettings;
            } else if (parsed.preferences && typeof parsed.preferences === 'object') {
              incoming = parsed.preferences;
            } else {
              incoming = parsed;
            }
          }
          ...
          if (!incoming || typeof incoming !== 'object' || Array.isArray(incoming)) {
            throw new Error('Unrecognized settings format in JSON.');
          }

          // Build validated & sanitized settings object
          const settingsToRestore = {};

          // 1. Theme & Colors
          if (typeof incoming.mode === 'string') {
            settingsToRestore.mode = incoming.mode.toLowerCase() === 'dark' ? 'dark' : 'light';
          }
          if (typeof incoming.accent === 'string' && incoming.accent.startsWith('#')) {
            settingsToRestore.accent = incoming.accent;
          }
          if (typeof incoming.secondary === 'string' && incoming.secondary.startsWith('#')) {
            settingsToRestore.secondary = incoming.secondary;
          }
          if (typeof incoming.customBgColor === 'string') {
            settingsToRestore.customBgColor = incoming.customBgColor;
          }

          // 2. Display & Zoom
          if (incoming.ui_zoom !== undefined) {
            settingsToRestore.ui_zoom = Math.max(70, Math.min(130, parseInt(incoming.ui_zoom, 10) || 95));
          }
          if (incoming.font_scale !== undefined) {
            settingsToRestore.font_scale = Math.max(70, Math.min(130, parseInt(incoming.font_scale, 10) || 100));
          }

          // 3. AI Pruner & Vault
          if (incoming.limit !== undefined) {
            settingsToRestore.limit = Math.max(1, parseInt(incoming.limit, 10) || 5);
          }
          if (incoming.pruningEnabled !== undefined || incoming.enabled !== undefined) {
            const isPruning = Boolean(incoming.pruningEnabled ?? incoming.enabled);
            settingsToRestore.pruningEnabled = isPruning;
            settingsToRestore.enabled = isPruning;
          }
          if (incoming.vaultEnabled !== undefined) {
            settingsToRestore.vaultEnabled = Boolean(incoming.vaultEnabled);
          }
          if (typeof incoming.chat_pruning_mode === 'string') {
            settingsToRestore.chat_pruning_mode = incoming.chat_pruning_mode === 'tail' ? 'tail' : 'sliding_window';
          }

          // 4. YouTube Cleaner
          if (incoming.yt_feed_limit_enabled !== undefined) {
            settingsToRestore.yt_feed_limit_enabled = Boolean(incoming.yt_feed_limit_enabled);
          }
          if (incoming.yt_feed_limit !== undefined) {
            settingsToRestore.yt_feed_limit = parseInt(incoming.yt_feed_limit, 10) || 12;
          }
          if (incoming.yt_not_interested_enabled !== undefined) {
            settingsToRestore.yt_not_interested_enabled = Boolean(incoming.yt_not_interested_enabled);
          }

          // 5. Flow Speed Controller
          if (incoming.flow_speed_enabled !== undefined) {
            settingsToRestore.flow_speed_enabled = Boolean(incoming.flow_speed_enabled);
          }
          if (incoming.flow_speed_val !== undefined) {
            settingsToRestore.flow_speed_val = Math.max(0.25, Math.min(5.0, parseFloat(incoming.flow_speed_val) || 1.0));
          }
          if (incoming.flow_speed_last_custom !== undefined) {
            settingsToRestore.flow_speed_last_custom = parseFloat(incoming.flow_speed_last_custom) || 1.5;
          }
          if (typeof incoming.flow_speed_cycle_list === 'string') {
            settingsToRestore.flow_speed_cycle_list = incoming.flow_speed_cycle_list.trim();
          }
          if (typeof incoming.flow_speed_shortcut_reset === 'string') {
            settingsToRestore.flow_speed_shortcut_reset = incoming.flow_speed_shortcut_reset.trim();
          }
          if (typeof incoming.flow_speed_shortcut_cycle === 'string') {
            settingsToRestore.flow_speed_shortcut_cycle = incoming.flow_speed_shortcut_cycle.trim();
          }
          if (typeof incoming.flow_speed_shortcut_up === 'string') {
            settingsToRestore.flow_speed_shortcut_up = incoming.flow_speed_shortcut_up.trim();
          }
          if (typeof incoming.flow_speed_shortcut_down === 'string') {
            settingsToRestore.flow_speed_shortcut_down = incoming.flow_speed_shortcut_down.trim();
          }
          if (incoming.flow_speed_show_hud !== undefined) {
            settingsToRestore.flow_speed_show_hud = Boolean(incoming.flow_speed_show_hud);
          }
          if (incoming.flow_speed_badge_enabled !== undefined) {
            settingsToRestore.flow_speed_badge_enabled = Boolean(incoming.flow_speed_badge_enabled);
          }

          // 6. RippleFrame Settings
          if (incoming.rf_enabled !== undefined) {
            settingsToRestore.rf_enabled = Boolean(incoming.rf_enabled);
          }
          if (typeof incoming.rf_capture_scope === 'string') {
            settingsToRestore.rf_capture_scope = incoming.rf_capture_scope === 'viewport' ? 'viewport' : 'full';
          }
          if (incoming.rf_scroll_delay !== undefined) {
            settingsToRestore.rf_scroll_delay = Math.max(300, parseInt(incoming.rf_scroll_delay, 10) || 550);
          }

          // 7. Card Collapsed States
          ['limit_card_collapsed', 'aesthetics_card_collapsed', 'display_card_collapsed', 'yt_card_collapsed', 'flow_speed_card_collapsed', 'rf_card_collapsed'].forEach(k => {
            if (incoming[k] !== undefined) {
              settingsToRestore[k] = Boolean(incoming[k]);
            }
          });

          // 8. Collections (Presets & Disabled Sites)
          if (Array.isArray(incoming.disabledSites)) {
            settingsToRestore.disabledSites = incoming.disabledSites;
          }
          if (Array.isArray(incoming.pinnedPresets)) {
            settingsToRestore.pinnedPresets = incoming.pinnedPresets;
          }

          // Copy any other custom properties that aren't binary/temporary caches
          for (const [k, v] of Object.entries(incoming)) {
            if (
              settingsToRestore[k] === undefined &&
              k !== 'latest_rippleframe_capture' &&
              k !== 'pendingTransferContext' &&
              !k.startsWith('temp_') &&
              !k.startsWith('cache_')
            ) {
          ...
          // Persist cleanly to chrome.storage.local
          chrome.storage.local.set(settingsToRestore, () => {
            loadSettings();
            syncVersionTag();
            importBtn.innerHTML = `<span>✅ Imported!</span>`;
            setTimeout(() => { importBtn.innerHTML = origHtml; }, 1600);
            alert('✅ Caspian Settings successfully imported and applied!');
          });
        } catch (err) {
          console.error('[Caspian Settings Import Error]', err);
