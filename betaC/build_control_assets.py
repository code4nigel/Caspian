...
sys.path.append(r'D:\Projects\Chatgpt Pruner\Caspian-Beta-A\assets')
from site_icons import GPT_ICON_B64, GEMINI_ICON_B64, GOOGLE_ICON_B64, YOUTUBE_ICON_B64

dev_avatar_path = r'D:\Projects\Chatgpt Pruner\Caspian-Android\assets\dev_avatar.txt'
dev_avatar_b64 = ""
if os.path.exists(dev_avatar_path):
    with open(dev_avatar_path, 'r', encoding='utf-8') as f:
...
<head>
  ...
  <link rel="stylesheet" href="browser_control.css">
  <script>
    (function() {{
      try {{
...
<body>
  ...
  <div id="bottom-sheet" class="caspian-bottom-sheet active" style="transform: translateY(0);">
    ...
    <div class="sheet-header">
      <div class="sheet-brand">
        ...
        <div style="display: flex; flex-direction: column;">
          <span class="sheet-brand-name">CASPIAN FLOW</span>
    ...
    <div id="tab-pane-engine" class="tab-pane active">
      ...
      <div id="card-chat-limit" class="m3-card engine-card expandable" style="margin-top: 14px;">
        ...
        <!-- Accordion Body: Contains Virtualization Mode & Visible Messages Limit selector -->
        <div id="chat-limit-body" style="display: block; margin-top: 12px; border-top: 1px solid var(--border-glass); padding-top: 10px;">
          <div class="limit-section-title" style="margin-bottom: 6px;">VIRTUALIZATION PRUNING MODE</div>
          <div class="pill-grid" style="grid-template-columns: 1fr 1fr; gap: 6px; margin-bottom: 12px;">
            <button class="pruner-mode-pill active" data-mode="sliding_window" id="btn-mode-sliding" style="font-size: 10px; padding: 6px 4px;">🪟 Sliding Window</button>
            <button class="pruner-mode-pill" data-mode="tail" id="btn-mode-tail" style="font-size: 10px; padding: 6px 4px;">📌 Tail Window</button>
          </div>

          <div class="limit-section-title" style="margin-bottom: 6px;">
            <span>VISIBLE MESSAGES LIMIT</span>
            <span>AUTO-PRUNE OLDER</span>
          </div>
          <div class="pill-grid" style="grid-template-columns: repeat(6, 1fr); gap: 4px;">
            <button class="limit-pill" data-val="2">2</button>
            <button class="limit-pill" data-val="4">4</button>
            <button class="limit-pill active" data-val="6">6</button>
            <button class="limit-pill" data-val="8">8</button>
            <button class="limit-pill" data-val="10">10</button>
            <button class="limit-pill" data-val="14">14</button>
            <button class="limit-pill" data-val="18">18</button>
            <button class="limit-pill" data-val="20">20</button>
            <button class="limit-pill" data-val="28">28</button>
            <button class="limit-pill" data-val="48">48</button>
                ...
                <div class="setting-row" style="margin-bottom: 8px; position: relative; z-index: 830;">
                  ...
                  <div class="caspian-select" id="select-export-gemini-normal" style="width: 160px;">
                    ...
                    <div class="caspian-select-options">
                      ...
                      <div class="caspian-select-option" data-val="crawler">Deep Component Crawler</div>
                    </div>
                  </div>
                </div>
      ...
      <div class="m3-card">
        ...
        <div class="dev-profile-box">
          ...
          <div class="dev-info-col">
            ...
            <a href="https://github.com/code4nigel" target="_blank" class="dev-github-link" onclick="if(window.CaspianBridge){{window.CaspianBridge.openExternalUrl('https://github.com/code4nigel');return false;}}">
        ...
        <div id="developer-options-card" class="m3-card" style="display: none; border: 1.5px solid #a855f7; background: rgba(168, 85, 247, 0.04); margin-top: 12px;">
          ...
          <!-- Secret Action Button Pulse Resonance & Cycle HUD Card -->
          <div style="margin-top: 10px; padding: 12px; background: linear-gradient(135deg, rgba(147, 51, 234, 0.12), rgba(15, 23, 42, 0.6)); border: 1.2px solid rgba(168, 85, 247, 0.4); border-radius: 14px; box-shadow: 0 4px 16px rgba(147, 51, 234, 0.15);">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
              <div style="display: flex; align-items: center; gap: 6px;">
                <span style="font-size: 14px;">⚡</span>
                <div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 8px;">
              <!-- Counter 1: Total Button Taps -->
              <div style="background: rgba(0, 0, 0, 0.3); border: 1px solid rgba(168, 85, 247, 0.25); border-radius: 10px; padding: 8px; text-align: center;">
                <div style="font-size: 8.5px; font-weight: 700; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Total Taps</div>
                <div id="dev-hud-total-clicks" style="font-size: 20px; font-weight: 900; color: #38bdf8; margin-top: 2px; text-shadow: 0 0 10px rgba(56, 189, 248, 0.5);">0</div>
                <div style="font-size: 8px; color: #94a3b8; margin-top: 2px;">Action Activations</div>
              </div>

              <!-- Counter 2: Open/Close Cycles (Total / 2) -->
              <div style="background: rgba(0, 0, 0, 0.3); border: 1px solid rgba(168, 85, 247, 0.25); border-radius: 10px; padding: 8px; text-align: center;">
                <div style="font-size: 8.5px; font-weight: 700; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Full Cycles</div>
                <div id="dev-hud-total-cycles" style="font-size: 20px; font-weight: 900; color: #ec4899; margin-top: 2px; text-shadow: 0 0 10px rgba(236, 72, 153, 0.5);">0</div>
                <div style="font-size: 8px; color: #94a3b8; margin-top: 2px;">Open & Close (Taps / 2)</div>
              </div>
            </div>

            <!-- Resonance Progress Bar -->
            <div style="margin-top: 8px;">
              <div style="display: flex; justify-content: space-between; font-size: 8.5px; color: var(--text-muted); margin-bottom: 3px;">
                <span>Resonance Synergy</span>
                <span id="dev-hud-synergy-pct">100%</span>
              </div>
              <div style="height: 4px; width: 100%; background: rgba(255, 255, 255, 0.1); border-radius: 2px; overflow: hidden;">
                <div id="dev-hud-progress-bar" style="height: 100%; width: 10%; background: linear-gradient(90deg, #38bdf8, #a855f7, #ec4899); border-radius: 2px; transition: width 0.5s ease;"></div>
              </div>
            </div>
          </div>

          <div class="m3-card-row" style="margin-top: 10px;">
            <div class="m3-card-left">
  ...
  <script src="browser_control.js"></script>
</body>
</html>
'''

gradle_path = r'D:\Projects\Chatgpt Pruner\betaC\app\build.gradle.kts'
vname = '1.0.3-BetaC'
...
out_file = r'D:\Projects\Chatgpt Pruner\betaC\app\src\main\assets\browser_control.html'
